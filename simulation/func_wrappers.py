...
def recursion_detector(max_recursion_limit:int|Callable=_MAX_RECURSION_FUNC):
    ...
    _max_recursion_limit = max_recursion_limit if isinstance(max_recursion_limit,int) else _MAX_RECURSION_FUNC
    ...
    def decorator(func:Callable):
        ...
        def wrapper(*args, **kwargs):
            ...
            if func._thread_locals.seen_times > _max_recursion_limit:
                raise RuntimeError(
                    f'Already called this function {_max_recursion_limit} times, check this is correct, if so increase the recursion limit')
    ...
    return decorator if isinstance(max_recursion_limit,int) else decorator(max_recursion_limit)
