...
# socketio_init  -> app_globals -> GreenPointsSimulation -> websocketstack -> socketio_init
...
# Declare Pool for Multiprocessing:
# If want to use multithreading instead, then just change import Pool to import ThreadPool
...
def testWsEventMemory():
    ...
    def _emitNEvents(N:int):
        for i in range(N):
            gpApp._emit_event(WebSocketServerResponseEvent.message_received ,{'data': f'test message {N}'})
    ...
    if not gpApp.initialised():
        ...
        if valid_retailer_name:
            ...
            if valid_retailer_strategy and valid_retailer_sustainability:
                ...
                abort(wrap_CORS_response(Response('Strategy / Sustainability enum name passed to simulation init request not found', status=404)))
        else:
            abort(wrap_CORS_response(Response('retailer_name passed to simulation init request not found', status=404)))
...
def adjust_retailer_in_sim():
    ...
    if 'simulation_id' in data:
        ...
        valid_simId = gpApp.getSimulationEnvironment(simulationId=simId) is not None
    else:
        statusId = 404
        abort(wrap_CORS_response(Response('retailer_name passed to adjust-retailer POST request not found', status=statusId)))
    ...
    if 'retailer_strategy' in data:
        ...
        valid_retailer_strategy = validate_retailer_strategy(retailerName, retailerStrategy)
        
    if 'retailer_sustainability' in data:
        retailerSustainability = data['retailer_sustainability']
        valid_retailer_strategy = validate_retailer_strategy(retailerName, retailerSustainability)
    ...
    if valid_simId and valid_retailer_strategy and valid_retailer_sustainability:
        ...
        statusId = 404
        abort(wrap_CORS_response(Response('Strategy / Sustainability enum name passed to simulation init request not found', status=statusId)))
...
def run_isolated_simulation():
    ...
    if isinstance(dual_pool,ThreadPool):
        if flaskHttpApp.config["ALLOW_THREADING"]:
            async_result = dual_pool.apply_async(gpApp.run_isolated_iteration, args=(), kwds={}, callback=None)
            tasks[taskId] = async_result
            #BUG: This will cause a memory leak given enough time as this store should be on disk on a db....
            #  we can just delete old tasks by popping them once accessed as they should only be asked fro once if one client has the taskid.
            taskPresenters[taskId] = lambda res: res.to_json()
...
def run_simulation():
    if isinstance(dual_pool,ThreadPool):
        ...
        if 'convergence_threshold' in data:
            kwds['convergence_threshold'] = float(data['convergence_threshold'])
        taskId = str(uuid.uuid4())
        simId = gpApp.simulationEnvironmentToken()
        if flaskHttpApp.config["ALLOW_THREADING"]:
            async_result = dual_pool.apply_async(lambda : gpApp.run_full_simulation(simulationId=simId, **kwds), callback=None)    
            tasks[taskId] = async_result
            #BUG: This will cause a memory leak given enough time as this store should be on disk on a db....
            #  we can just delete old tasks by popping them once accessed as they should only be asked fro once if one client has the taskid.
            taskPresenters[taskId] = lambda res: res.to_json()
...
def get_task_result():
    ...
    if taskId in tasks:
        ...
        if async_result.successful():
            ...
            response = wrap_CORS_response(make_response(taskPresenters[taskId](result_df)))
            # response.headers['Access-Control-Allow-Origin'] = request.origin
            return response
        else:
...
def adjust_purchases_delay_seconds(delay:float=1.0):
    try:
        ...
        response = wrap_CORS_response(Response({'message': 'success'}, status=HTTPStatus.ACCEPTED))
    except Exception as e:
        print(e)
        raise e
...
def get_all_accepted_ws_event_names():
    eventNames = list([w.value for w in  WebSocketClientEvent.__members__.values()])
...
def get_all_accepted_ws_response_event_names():
    eventNames = dict({name:value for (name,value) in  GPStrategyMultiplier.__members__.items()})
    return wrap_CORS_response(Response({'data': eventNames}, status=HTTPStatus.OK))
...
def getSustainabilityValueMap():
    eventNames = list([w.value for w in  RetailerSustainabilityIntercept.__members__.values()])
    return wrap_CORS_response(Response({'data': eventNames}, status=HTTPStatus.OK))
